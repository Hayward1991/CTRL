...
object Planner{
 ...
 private val dayStart=LocalTime.of(5,0)
 ...
 private fun inBlackout(time:LocalTime)=time>=nightStart||time<dayStart
 private fun anchor(t:CtrlTask):CtrlTask=when(t.recurrenceKey){
  "morning_care"->t.copy(start=if(t.start>="05:00"&&t.start<"10:00")t.start else "08:00",protected=true)
  "breakfast"->t.copy(start=if(t.start>="05:00"&&t.start<"10:00")t.start else "08:30",protected=true)
  ...
  else->if(inBlackout(t.localStart().toLocalTime())) t.copy(start="05:00",status=if(t.status==Status.ACTIVE||t.status==Status.PAUSED)t.status else Status.QUEUED) else t
 }
 private fun blocked(tasks:List<CtrlTask>,calendar:List<CalendarBlock>,date:LocalDate):MutableList<Pair<LocalDateTime,LocalDateTime>>{
  ...
  b+=date.atStartOfDay() to date.atTime(5,0)
  b+=date.atTime(22,0) to date.plusDays(1).atStartOfDay()
 ...
 fun planDay(tasks:List<CtrlTask>,calendar:List<CalendarBlock>,date:LocalDate=LocalDate.now()):PlanResult{
  val normalized=tasks.map(::anchor);val b=blocked(normalized,calendar,date);val out=normalized.toMutableList();var cursor=date.atTime(5,0);val hardStop=date.atTime(22,0);var over=0
  fun hit(s:LocalDateTime,e:LocalDateTime)=b.firstOrNull{s<it.second&&e>it.first}
  fun slot(from:LocalDateTime,min:Int,latest:LocalDateTime):LocalDateTime?{var s=maxOf(from,date.atTime(5,0));while(s.plusMinutes(min.toLong())<=latest){val h=hit(s,s.plusMinutes(min.toLong()));if(h==null)return s;s=h.second.plusMinutes(CtrlRules.BUFFER_MIN.toLong())};return null}
  ...
  movable.forEach{t->
   val desired=maxOf(cursor,t.localStart(),date.atTime(5,0));val latest=if(t.area==Area.WORK)date.atTime(17,30) else hardStop;val s=slot(desired,t.minutes,latest)
   if(s==null){val i=out.indexOfFirst{it.id==t.id};if(i>=0&&t.status==Status.DELAYED)out[i]=t.copy(date=date.plusDays(1).toString(),start="05:00",status=Status.QUEUED) else if(t.priority!=Priority.SOMEDAY)over+=t.minutes}
